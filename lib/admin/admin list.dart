...
class _AdminListState extends State<AdminList> {
  ...
  Future<void> deleteRowAPI(String TypeId) async {
    try {
      String apiUri =
    '$apicall/user/delete_usermaster_by_id/$TypeId';
  ...
  Future<void> fetchProducts(int page, int itemsPerPage) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData is List) {
          products = jsonData.map((item) => UserResponse.fromJson(item)).toList();
  ...
  void _filterAndPaginateProducts() {
    filteredData1 = productList.where((product) {
      final matchesSearchText =
          product.userId.toLowerCase().contains(_searchText.toLowerCase());
      final matchesSearchText1 =
      product.userName.toLowerCase().contains(_searchText1.toLowerCase());


      return matchesSearchText && matchesSearchText1;
      ...
      if (sortDirection == 'asc') {
        ...
        filteredData1.sort((a, b) {
          if (columnIndex == 0) {
            ...
            return b.userName
                .compareTo(a.userName); // Reverse the comparison
          } else if (columnIndex == 2) {
            return b.role
                .compareTo(a.role); // Reverse the comparison
          } else if (columnIndex == 3) {
            return b.companyName
                .compareTo(a.companyName); // Reverse the comparison
          } else if (columnIndex == 4) {
            return b.location
                .compareTo(a.location); // Reverse the comparison
          } else {
            return 0;
          }
        });
