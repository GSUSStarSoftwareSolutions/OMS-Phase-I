...
class CusInvoiceList extends StatefulWidget {
  const CusInvoiceList({super.key});
  @override
  State<CusInvoiceList> createState() => _CusInvoiceListState();
}

class _CusInvoiceListState extends State<CusInvoiceList> {
  ...
  List<String> columns1 = ['Invoice ID','Order ID','Order Date' ,'Delivery Status','Amount','Payment Status', 'PDF'];
  List<String> columns = ['','Invoice No','Date','Order ID','Location' ,'Status','Amount'];
  List<double> columnWidths = [110, 110, 110, 135, 130,140, 100];
  ...
  List<Widget> _buildMenuItems(BuildContext context) {
    return [
      ...
      _buildMenuItem('Invoice', Icons.document_scanner_rounded, Colors.blueAccent, '/Customer_Invoice_List'),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData != null) {
          ...
          List<OrderDetail> matchedCustomers = products.where((customer) {
            return customer.customerId!.trim().toLowerCase() == userId.trim().toLowerCase();
          }).toList();

          if(matchedCustomers.isNotEmpty){
            setState(() {
              print(matchedCustomers);
 ...
 //   orderId = _selectedProduct!.orderId!;
    try {
      ...
      if (orderDetails != null) {
        final Uint8List pdfBytes = await Invoicepdf1(orderDetails as OrderDetail);
      ...
      if (sortDirection == 'asc') {
        ...
        filteredData.sort((a, b) {
          if (columnIndex == 0) {
            ...
            return b.Payment!.compareTo(a.Payment!); // Reverse the comparison
          }else {
            return 0;
          }
        });
                        ...
                        label: Stack(
                          children: [
                            Container(
                              ...
                              width: columnWidths[columns1.indexOf(column)], // Dynamic width based on user interaction
                              child: Row(
                                ...
                                children: [
                                  ...
                                  if (columns1.indexOf(column) < columns1.length - 1)
                                    IconButton(
                                      icon: _sortOrder[columns1.indexOf(column)] == 'asc'
                                          ?SizedBox(width: 12,
                                          child: Image.asset("images/sort.png",color: Colors.grey,))
                                          : SizedBox(width: 12,child: Image.asset("images/sort.png",color: Colors.blue,)),
                                      onPressed: () {
                                        setState(() {
                                          _sortOrder[columns1.indexOf(column)] = _sortOrder[columns1.indexOf(column)] == 'asc' ? 'desc' : 'asc';
                                          _sortProducts(columns1.indexOf(column), _sortOrder[columns1.indexOf(column)]);
                                        });
                                      },
                                    ),
                                  if (columns1.indexOf(column) < columns1.length - 1)
                                    ...
                                    MouseRegion(
                                      ...
                                      child: GestureDetector(
                                          onHorizontalDragUpdate: (details) {
                                            ...
                                            setState(() {
                                              columnWidths[columns1.indexOf(column)] += details.delta.dx;
                                              columnWidths[columns1.indexOf(column)] =
                                                  columnWidths[columns1.indexOf(column)].clamp(50.0, 300.0);
                                            });
                                            // setState(() {
