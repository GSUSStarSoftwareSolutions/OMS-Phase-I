...
import 'package:btb/widgets/productdata.dart';
...
void main() => runApp(CusOpenorder());

class CusOpenorder extends StatefulWidget {
  const CusOpenorder({super.key});
//  final ord.Product? product;
  @override
  State<CusOpenorder> createState() => _CusOpenorderState();
}

class _CusOpenorderState extends State<CusOpenorder> {
    ...
    try {
      ...
      if(token == " "){
        ...
        if (response.statusCode == 200) {
          ...
          if (jsonData is List) {
            filteredData =
                jsonData.map((item) => detail.fromJson(item)).toList();
          } else if (jsonData is Map && jsonData.containsKey('body')) {
            final body = jsonData['body'];
            if (body != null) {
              filteredData =
                                                  ...
                                                  children: [
                                                    PaginationControls(
                                                      ...
                                                      product.deliveryStatus ==
                                                          'Not Started' && product.CusId == userId)
                                                          .length /
                                                          itemsPerPage).ceil(),
            ...
            child: DataTable(
                ...
                rows: List.generate(
                    math.min(itemsPerPage, filteredData1
                        .where((detail) => detail.deliveryStatus == 'Not Started' && detail.CusId == userId)
