  ...
  Future<void> _fetchOrderDetails(String cusId) async {
    ...
    if (token == " ") {
      ...
      if (cusMasterResponse.statusCode == 200) {
        ...
        print(cusData);
        bool isMatched =
            cusData.any((invoice) => invoice['customerName'] == customerId);
        if (isMatched) {
          var matchedCustomer = cusData
              .firstWhere((customer) => customer['customerName'] == customerId);
          setState(() {
            ContactPersonController.text =
                matchedCustomer['customerName'] ?? '';
            CustomerIdController.text =
                matchedCustomer['customerId'] ?? ''; // Safely assign the value
            ContactNumberController.text =
                matchedCustomer['contactNo']?.toString() ??
                    ''; // Convert to string and safely assign
                          ...
                          decoration: InputDecoration(
                            ...
                            border: OutlineInputBorder(
                              ...
                              borderSide: const BorderSide(
                                  color:
                                      Color(0xFFBBDEFB)), // Added blue border
                            ),
                            enabledBorder: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(5.0),
                              borderSide: const BorderSide(
                                  color: Colors.grey), // Added blue border
                            ),
                                            ...
                                            child: DataTable(
                                              border: const TableBorder(
                                                ...
                                                horizontalInside: BorderSide(
                                                    width: 1,
                                                    color: Colors.grey),
                                                // horizontal border inside the table
                                                    ...
                                                    onPressed: () {
                                                      String validateFields() {
                                                        ...
                                                        if (EmailIdController.text.isEmpty) {
                                                          return 'Please fill the Email ID field.';
                                                        }
                                                        if (!RegExp(r'^[\w-]+(\.[\w-]+)*@[a-zA-Z0-9-]+\.(com|in|net)$').hasMatch(EmailIdController.text)) {
                                                          return 'Enter a valid E-mail address.';
                                                        }
                                                        if (ShippingAddress.text.isEmpty) {
                                                          return 'Please fill the Shipping address.';
                                                        }
                                                        return ''; // Return an empty string if no validation error
                                                      }

// Call the validation and handle the result
