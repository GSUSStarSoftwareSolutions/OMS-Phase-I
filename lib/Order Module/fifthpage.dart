...
class _FifthPageState extends State<FifthPage> {
  ...
  Future<void> updateOrderDetailsAndNavigate() async {
    ...
    if (response.statusCode == 200) {
      ...
      final newList = jsonData.map((detail) => OrderDetail(
    ...
    if (response.statusCode == 200) {
     ...
     // String orderId;
      ...
      if (details != null) {
       ...
       // await callApi2();
        List<OrderDetail> orderDetails = filteredData.map((detail) => OrderDetail(
          orderId: detail.orderId,
          orderDate: detail.orderDate,
  ...
  double _calculateTotalAmount(Product product) {
    ...
    double discountAmount = basicAmount * (double.parse(product.discount.replaceAll('%', '')) / 100);
   // double roundedDiscountAmount = discountAmount.roundToDouble();
    print(discountAmount);

    // Calculate GST amount
    double gstAmount = basicAmount * (double.parse(product.tax.replaceAll('%', '')) / 100);
